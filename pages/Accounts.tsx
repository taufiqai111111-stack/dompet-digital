...
const AccountForm: React.FC<{ 
  ...
  const [balance, setBalance] = useState(initialData ? String(initialBalance ?? 0) : '0');

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const balanceNum = parseFloat(balance) || 0;
    onSave({ name, type }, balanceNum);
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input type="number" step="any" id="balance" value={balance} onChange={(e) => setBalance(e.target.value)} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" />
      </div>
