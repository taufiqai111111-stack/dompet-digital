...
const AccountForm: React.FC<{ onSave: (account: Omit<Account, 'id'>, initialBalance: number) => void; initialData?: Account | null }> = ({ onSave, initialData }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({ name, type, balance: initialData?.balance || 0 }, initialData ? 0 : balanceNum); // Initial balance only on create
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <input type="number" id="balance" value={balance} onChange={(e) => setBalance(e.target.value)} disabled={!!initialData} className="mt-1 block w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100" />
        {initialData && <p className="text-xs text-gray-500 mt-1">Saldo awal tidak dapat diubah. Sesuaikan melalui halaman Transaksi.</p>}
      </div>
...
const Accounts: React.FC = () => {
  const { accounts, addAccount, updateAccount, deleteAccount, isAccountInUse } = useAppContext();
  const [isModalOpen, setModalOpen] = useState(false);
  const [editingAccount, setEditingAccount] = useState<Account | null>(null);
  const [deletingId, setDeletingId] = useState<string | null>(null);

  const handleSave = (accountData: Omit<Account, 'id'>, initialBalance: number) => {
    if (editingAccount) {
  ...
  return (
    <div className="space-y-6">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setModalOpen(false)} title={editingAccount ? 'Edit Rekening' : 'Tambah Rekening'}>
        <AccountForm onSave={handleSave} initialData={editingAccount} />
      </Modal>

      <Modal isOpen={!!deletingId} onClose={() => setDeletingId(null)} title="Konfirmasi Hapus">
        <div>
          <p className="text-gray-600 mb-4">Apakah Anda yakin ingin menghapus rekening ini?</p>
