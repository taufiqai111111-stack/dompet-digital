...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, setIsOpen }) => {
  // FIX: Refactor NavItem to use a render prop for NavLink's children.
  // This correctly scopes the `isActive` variable for conditional styling of both the link and the icon.
  // FIX: Explicitly type NavItem as a React.FC to allow TypeScript to correctly handle the 'key' prop.
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 w-64 bg-white h-full z-40 shadow-lg transform transition-transform md:relative md:translate-x-0 md:shadow-none ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="p-4 space-y-2">
          {/* FIX: Replaced spread props with explicit props to resolve TypeScript error with the 'key' prop. */}
