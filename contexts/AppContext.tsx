...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const [accounts, setAccounts] = useLocalStorage<Account[]>(`accounts_${userKey}`, []);
  const [platforms, setPlatforms] = useLocalStorage<Platform[]>(`platforms_${userKey}`, []);
  const [investments, setInvestments] = useLocalStorage<Investment[]>(`investments_${userKey}`, []);
  const [transactions, setTransactions] = useLocalStorage<Transaction[]>(`transactions_${userKey}`, []);
  const [receivables, setReceivables] = useLocalStorage<Receivable[]>(`receivables_${userKey}`, []);
  const [assets, setAssets] = useLocalStorage<Asset[]>(`assets_${userKey}`, []);
